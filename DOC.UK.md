...
Стрімляться обидва напрямки. Стиснене повідомлення спершу повністю
розпаковується; `NextWriter` надсилає фрагменти, щойно вони заповнюються, тож
повідомлення, більше за пам'ять, - це просто питання запису.
...
Повідомлення, що вміщається в один фрагмент (32 КіБ), і далі надсилається одним
нефрагментованим кадром на `Close`, тож для звичайних повідомлень на дроті нічого
не змінилося. Більші йдуть у міру запису, і в пам'яті тримається щонайбільше
фрагмент.

Одночасно може тривати лише одне повідомлення. Від першого фрагмента до `Close`
інший `WriteMessage` чи `NextWriter` повертає `ErrMessageInFlight`, бо його кадри
прочитались би як продовження відкритого. Control-кадри це не зачіпає - вони й далі
йдуть між фрагментами, тож ping, pong і close працюють під час довгого
повідомлення. Щоб завершити повідомлення, треба викликати `Close`.

`SetWriteLimit` і далі обмежує повідомлення цілком і відмовляє до того, як
будь-яка частина завеликого потрапить на дріт.

Reader дійсний для одного повідомлення: щойно `NextReader` чи `ReadMessage`
рушили далі, старий віддає `ErrStaleReader`, а не байти наступного
повідомлення. `NextWriter` відмовляє на з'єднанні, чиї записи вже впали або яке
надіслало close, замість прийняти ціле повідомлення й впасти на `Close`; а
`Close` повертає той самий результат, скільки б разів його не викликали, тож
відкладений `Close` після невдалого не звітує про успіх.
...
- `*CloseError{Code, Text}` - як завершилося з'єднання. Використовуйте
  ...
  Будь-яке завершення без closing-handshake дає код 1006 - кінець потоку, потік,
  обірваний посеред кадру, і мережевий збій на кшталт reset чи broken pipe - із
  початковою помилкою як причиною, тож `errors.Is(err, io.EOF)` і подібні
  працюють. Дедлайн читання завершенням не є: він лишається тим самим timeout,
  бо з'єднання на місці, а дедлайн поставили ви. 1006 - локальне спостереження,
  на дріт не надсилається.
...
- `ErrStaleReader` - reader належить повідомленню, яке з'єднання вже минуло.
- `ErrMessageInFlight` - потокове повідомлення вже має кадри на дроті; закрийте
  його writer перед початком наступного.
- `ErrConfig` - опції передано значення, непридатне для цього пакета.

...
Вікно - повні 32 КіБ і не узгоджується. Пропозицію стискати цим боком із меншим
вікном (`server_max_window_bits`) ми відхиляємо, і з'єднання працює без
стиснення - RFC 7692 це дозволяє; `client_max_window_bits`, який шлють браузери,
приймається і не відлунюється. Стандартна бібліотека не дає керувати розміром
вікна deflate, а сторонньої залежності заради цього пакет не бере.
`Conn.CompressionEnabled` повідомляє, що узгодилось насправді.
